  ...
  return (
    <section id="projects" className="mx-auto mt-20 w-full max-w-5xl px-6 md:px-8">
      <header className="mb-10 flex flex-col gap-2">
        <span className="flex items-center gap-2 text-sm uppercase tracking-[0.3em] text-accent/80">
            ...
            <div className="glass rounded-[calc(theme(borderRadius.3xl)-4px)] p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs font-medium">
                {project.tech.map((tech) => (
                  <span key={tech} className="rounded-full bg-slate-800/80 border border-slate-600/50 px-3 py-1.5 text-slate-100">
                    {tech}
                  </span>
                ))}
              </div>
